...
use crate::coord::{
    Coordinator, CopyToStage, CopyToStageFinish, CopyToStageOptimize, CopyToStageRealTimeRecency,
    CopyToStageTimestamp, CopyToStageValidate, ExecuteContext, Message, PlanValidity,
...
impl Coordinator {
    /// Sequence a copy to, determining a timestamp and the most efficient dataflow interaction.
    ///
    /// CopyTos are sequenced by assigning a timestamp for evaluation, and then determining and
    /// deploying the most efficient evaluation plan. The copy to could evaluate to a constant,
    ...
    pub(crate) async fn sequence_copy_to(
        ...
        self.sequence_copy_to_stage(
            ...
            CopyToStage::Validate(CopyToStageValidate {
                plan,
                target_cluster,
            }),
        )
    ...
    /// Processes as many `copyto` stages as possible.
    #[tracing::instrument(level = "debug", skip_all)]
    pub(crate) async fn sequence_copy_to_stage(
        ...
        loop {
            ...
            // Always verify copyto validity. This is cheap, and prevents programming errors
            ...
            (ctx, stage) = match stage {
                CopyToStage::Validate(stage) => {
                    let next =
                        return_if_err!(self.copy_to_stage_validate(ctx.session_mut(), stage), ctx);

                    (ctx, CopyToStage::Timestamp(next))
                }
                CopyToStage::Timestamp(stage) => {
                    self.copy_to_stage_timestamp(ctx, root_otel_ctx.clone(), stage)
                        .await;
                    return;
                }
                CopyToStage::Optimize(stage) => {
                    self.copy_to_stage_optimize(ctx, root_otel_ctx.clone(), stage)
                        .await;
                    return;
                }
                CopyToStage::RealTimeRecency(stage) => {
                    match self.copy_to_stage_real_time_recency(ctx, root_otel_ctx.clone(), stage) {
                        Some((ctx, next)) => (ctx, CopyToStage::Finish(next)),
                        None => return,
                    }
                }
                CopyToStage::Finish(stage) => {
                    let res = self.copy_to_stage_finish(&mut ctx, stage).await;
                    ctx.retire(res);
                    return;
                }
            }
    ...
    fn copy_to_stage_validate(
    ...
    ) -> Result<CopyToStageTimestamp, AdapterError> {
        let plan::CopyToPlan {
            source,
            when,
            finishing,
        ...
        let optimizer = optimize::copy_to::Optimizer::new(
            Arc::clone(&catalog),
            compute_instance,
            finishing.clone(),
            view_id,
    ...
    /// needed.
    #[tracing::instrument(level = "debug", skip_all)]
    async fn copy_to_stage_timestamp(
        &mut self,
        ctx: ExecuteContext,
        root_otel_ctx: OpenTelemetryContext,
        CopyToStageTimestamp {
            validity,
            source,
            source_ids,
            when,
        ...
        let build_optimize_stage = move |oracle_read_ts: Option<Timestamp>| -> CopyToStageOptimize {
            CopyToStageOptimize {
                validity,
                source,
                source_ids,
            ...
            Some(timeline) => {
                ...
                if let Some(shared_oracle) = shared_oracle {
                    ...
                    mz_ore::task::spawn(|| "linearized timestamp task", async move {
                        ...
                        let stage = CopyToStage::Optimize(stage);
                        // Ignore errors if the coordinator has shut down.
                        let _ = internal_cmd_tx.send(Message::CopyToStageReady {
                            ctx,
    ...
    #[tracing::instrument(level = "debug", skip_all)]
    async fn copy_to_stage_optimize(
        &mut self,
        ctx: ExecuteContext,
        root_otel_ctx: OpenTelemetryContext,
        mut stage: CopyToStageOptimize,
    ) {
        ...
        let span = tracing::debug_span!("optimize copy to task");
        ...
        mz_ore::task::spawn_blocking(
            ...
            move || {
                ...
                match Self::optimize_copy_to(ctx.session(), stats, id_bundle, stage) {
                    Ok(stage) => {
                        let stage = CopyToStage::RealTimeRecency(stage);
                        // Ignore errors if the coordinator has shut down.
                        let _ = internal_cmd_tx.send(Message::CopyToStageReady {
                            ctx,
                            otel_ctx: root_otel_ctx,
    ...
    ) -> Result<CopyToStageRealTimeRecency, AdapterError> {
    ...
    #[tracing::instrument(level = "debug", skip_all)]
    fn copy_to_stage_real_time_recency(
        &mut self,
        ctx: ExecuteContext,
        root_otel_ctx: OpenTelemetryContext,
        CopyToStageRealTimeRecency {
            validity,
            source_ids,
            id_bundle,
            when,
    ...
    ) -> Option<(ExecuteContext, CopyToStageFinish)> {
        match self.recent_timestamp(ctx.session(), source_ids.iter().cloned()) {
            Some(fut) => {
                ...
                task::spawn(|| "real_time_recency_copy_to", async move {
    ...
    async fn copy_to_stage_finish(
        ...
        let planned_copy_to = PlannedPeek {
            plan: PeekPlan::SlowPath(plan),
        ...
        if let Some(transient_index_id) = match &planned_copy_to.plan {
            peek::PeekPlan::FastPath(_) => unreachable!("COPY TO does not have fast paths"),
        ...
        // Implement the copy to, and capture the response.
        let resp = self
            .implement_peek_plan(
                ctx.extra_mut(),
                planned_copy_to,
    ...
    /// if necessary.
    #[tracing::instrument(level = "debug", skip_all)]
    pub(super) async fn sequence_copy_to_timestamp(
        &mut self,
        session: &mut Session,
        when: &QueryWhen,
        ...
        // XXX: check transaction behavior
        ...
        // // We only track the copy to in the session if the query doesn't use AS
        ...
        //     session.add_transaction_ops(TransactionOps::CopyTos {
