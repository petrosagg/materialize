...
mod test {
    ...
    fn gh_9160_repro() {
        timely::execute_directly(|worker| {
            let (mut input_handle, mut persist_handle) =
                worker.dataflow::<MzTimestamp, _, _>(|scope| {
                    ...
                    scope.scoped::<(MzTimestamp, Subtime), _, _>("upsert", |scope| {
                        ...
                        let input = input.as_collection().inspect(|((_, row, _), ts, diff)| {
                            println!(" input: ({:?}, {:?}, {})", row, ts.0, diff)
                        });
                        ...
                        output.inspect(|(row, ts, diff)| {
                            println!("output: ({:?}, {:?}, {})", row, ts.0, diff)
                        });
